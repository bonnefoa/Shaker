...
abstractTestListFiles fli pred = monadicIO test
  where test = do
...
prop_listFiles fli = abstractTestListFiles fli{ignore=[]} (\a b -> length b>2)

prop_listFilesWithIgnoreAll :: FileListenInfo -> Property
prop_listFilesWithIgnoreAll fli = abstractTestListFiles fli{ignore= [".*"], include=[]} (\a b -> b==[])
